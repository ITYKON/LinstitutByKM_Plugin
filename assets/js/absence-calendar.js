...
class AbsenceCalendar {
  constructor() {
    ...
    if (!window.ib_absence_ajax) {
      console.error(
        "Erreur: La variable ib_absence_ajax n'est pas définie. Vérifiez que le script est correctement localisé."
      );
      return;
    }
  ...
  loadEmployees() {
    ...
    if (employeeSelect) {
      this.employees = Array.from(employeeSelect.options)
        .map((option) => ({
          id: option.value,
          name: option.text,
        }))
  ...
  // Ajout du gestionnaire de soumission une seule fois
  bindFormSubmitOnce() {
    const form = document.getElementById("absence-form");
    if (form && !form._ibSubmitHandlerAdded) {
      ...
      form._ibSubmitHandlerAdded = true;
    }
  }
  ...
  renderCalendar() {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      html += `
                <div class="${dayClass}" data-date="${this.formatDate(
        currentDate
      )}" ${
        !isPastDay
          ? `onclick="absenceCalendar.selectDate('${this.formatDate(
              currentDate
            )}')"`
          : ""
      }>
  ...
  validateAbsenceDates() {
    ...
    if (startDate < today) {
      ...
      startDateMessage.textContent =
        "La date de début ne peut pas être antérieure à aujourd'hui";
      startDateInput.parentNode.insertBefore(
        startDateMessage,
        startDateInput.nextSibling
      );
      return false;
    }
    ...
    try {
      ...
      try {
        ...
        if (result.success) {
          ...
          alert(
            "Erreur: " + (result.data?.message || "Échec de l'enregistrement")
          );
        }
      } catch (e) {
        ...
        if (response.ok) {
          ...
          alert(
            "Erreur inattendue du serveur. Voir la console pour plus de détails."
          );
        }
      }
    } catch (error) {
  ...
  async deleteAbsence() {
    const absenceId = document
      .getElementById("absence-form")
      .querySelector('input[name="absence_id"]').value;

    if (
      !absenceId ||
      !confirm("Êtes-vous sûr de vouloir supprimer cette absence ?")
    ) {
      return;
    }
