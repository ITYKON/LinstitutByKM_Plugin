...
<style>
  ...
  #services-grid {
    ...
    max-width: 100% !important;
    padding: 10px;
    /* Ajout d'un padding pour éviter le débordement  */
  }
...
<script>
  ...
  function detectCurrentStep() {
    ...
    if (document.querySelector('h2, h3, .step-title')) {
      ...
      // panier nes
      else if (titleText.includes('panier') || titleText.includes('date') || titleText.includes('heure') ||  titleText.includes('réservation')) {
        currentStep = 4;
    ...
    // nes Vérifier si on est sur la page du panier
    else if (document.querySelector('')) {
      currentStep = 5;
